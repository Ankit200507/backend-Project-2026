...
import {
    LayoutDashboard, Map, FileText, Plus, Building2,
    Shield, ChevronLeft, ChevronRight, Home
} from 'lucide-react';
        ...
        <aside style={{
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    flexShrink: 0, boxShadow: '0 4px 12px var(--accent-green-glow)',
                }}>
                    <Shield size={18} color="#0a0c10" strokeWidth={2.5} />
                </div>
                {!collapsed && (
                    <div>
                        <div style={{ fontFamily: 'Space Grotesk,sans-serif', fontWeight: 700, fontSize: 15, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
                            TerraLedger
                        </div>
                        <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 500, letterSpacing: '0.06em' }}>
                            LAND REGISTRY
                        </div>
                    </div>
                )}
            ...
            <nav style={{ flex: 1, padding: '16px 10px', display: 'flex', flexDirection: 'column', gap: 4, overflowY: 'auto' }}>
                ...
                <div style={{ marginTop: 8, borderTop: '1px solid var(--border-primary)', paddingTop: 8 }}>
                    <Link href="/" style={{
                        ...
                        textDecoration: 'none', transition: 'var(--transition)', whiteSpace: 'nowrap',
            ...
            <div style={{ padding: '12px 10px', borderTop: '1px solid var(--border-primary)', flexShrink: 0 }}>
                {!collapsed && (
                    <div style={{ marginBottom: 10, padding: '10px 14px', background: 'var(--bg-glass)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-primary)' }}>
                        <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                            Switch Role (Demo)
                        </div>
                        <div style={{ display: 'flex', gap: 6 }}>
                            <button onClick={() => switchRole('admin')} className={`btn btn-sm ${isAdmin ? 'btn-primary' : 'btn-secondary'}`} style={{ flex: 1, justifyContent: 'center', fontSize: 12 }}>
                                Admin
                            </button>
                            <button onClick={() => switchRole('user')} className={`btn btn-sm ${!isAdmin ? 'btn-primary' : 'btn-secondary'}`} style={{ flex: 1, justifyContent: 'center', fontSize: 12 }}>
                                User
                            </button>
                        </div>
                    </div>
                ...
                <div style={{
                    ...
                    borderRadius: 'var(--radius-md)', justifyContent: collapsed ? 'center' : 'flex-start',
                }}>
                    <div style={{
                        ...
                        fontSize: 13, fontWeight: 700, color: isAdmin ? 'var(--accent-purple)' : 'var(--accent-blue)',
                        flexShrink: 0,
                    }}>
